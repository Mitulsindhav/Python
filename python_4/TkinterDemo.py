...
def insert_data():
    if e_fname.get()=="" or e_lname.get()=="" or e_email.get()=="" or e_mobile.get()=="":
        msg.showinfo("Inserted Status ","All fields are Mandatory")
    else:
         conn=create_conn()
         cursor=conn.cursor()
...
def search_data():
    if e_id.get()=="":
        msg.showinfo("Search Status","Id is Mondatory")
    else:
        conn=create_conn()
        cursor=conn.cursor()
...
def delete_data():
    if e_id.get()=="":
        msg.showinfo("Delete status","Id is not found")
    else:
        conn=create_conn()
        cursor=conn.cursor()
        ...
        e_lname.delete(0,'end')
        e_email.delete(0,'end')
        e_mobile.delte(0,'end')
        msg.showinfo("Delete status","data delteted successfully")
